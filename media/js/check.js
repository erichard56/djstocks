...
$('document').ready(function() {
	...
	$('table#items-check tbody tr').click(function() {
		...
		if($('tr[data-id='+id+']').next().data('id') != 'y') {
			var cont = $("<tr style=\"display:none\" data-id=\"y\"><td></td><td></td><td></td><td><form method=\"post\" data-saveid=\""+id+"\" name=\""+text+"\"><input type=\"text\" class=\"num\" placeholder=\"Cant.\" /></form></td><td><input type=\"submit\" value=\"Guardar\" class=\"btn blue save "+text+"\" /></td><td></td></tr>");
	...
	function submit_check(from, t) {
		...
		if(val == '0' || val == '') {
			alert('Por favor, inserte un número de elementos para comprobar '+actpage);
			return false;
		}
		...
		$.post(phppage, {
			...
			if(data == '2') {
				...
				alert('210 Algo salió mal. Por favor, inténtelo de nuevo');
			}
		});
	}
	...
	$('form[name=searchf]').submit(function(evt) {
		...
		$.post('check-in.php', {
			...
			$('.loader').fadeOut(200);
			alert('211 Algo salió mal. Por favor, inténtelo de nuevo');
			return false;
		});
	});
