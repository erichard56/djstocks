$('document').ready(function() {
	...
	$('a[name=c5]').click(function(evt) {
		...
		if(confirm('¿Seguro que quieres eliminar este Depósito? Todos los artículos y los registros en este Depósito serán suprimidos también') == true) {
			...
			$.post('categories.php', {
				...
				if(data == '1')
					...
					alert('220 Algo salió mal. Por favor, inténtelo de nuevo');
			});
		}
	});
	...
	$('form[name=searchf]').submit(function(evt) {
		...
		$.post('categories.php', {
			...
			$('.loader').fadeOut(200);
			alert('221 Algo salió mal. Por favor, inténtelo de nuevo');
			return false;
		});
	});
