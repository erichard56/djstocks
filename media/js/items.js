$('document').ready(function() {
		...
		for(var i = 0; i < selected.length; i++) {
			...
			if($('input[type=checkbox][name=chbox][value="'+id+'"]').parent().parent().next().data('id') != 'y') {
				if(getViewportWidth() > 420)
					var cont = $("<tr style=\"display:none\" data-id=\"y\"><td></td><td></td><td></td><td>"+text+"</td><td><form method=\"post\" data-saveid=\""+id+"\" name=\""+act+"\"><input type=\"text\" class=\"num\" placeholder=\"Cant.\" /></form></td><td><input type=\"submit\" value=\"Save\" class=\"btn blue save "+act+"\" /></td><td></td></tr>");
				else if(getViewportWidth() <= 420 && getViewportWidth() > 370)
					var cont = $("<tr style=\"display:none\" data-id=\"y\"><td></td><td></td><td>"+text+"</td><td></td><td><form method=\"post\" data-saveid=\""+id+"\" name=\""+act+"\"><input type=\"text\" class=\"num\" placeholder=\"Cant.\" /></form></td><td><input type=\"submit\" value=\"Save\" class=\"btn blue save "+act+"\" /></td><td></td></tr>");
				else
					var cont = $("<tr style=\"display:none\" data-id=\"y\"><td></td><td></td><td>"+text+"</td><td></td><td><form method=\"post\" data-saveid=\""+id+"\" name=\""+act+"\"><input type=\"text\" class=\"num\" placeholder=\"Cant.\" /></form></td><td></td><td><input type=\"submit\" value=\"Guardar\" class=\"btn blue save "+act+"\" /></td><td></td></tr>");
	...
	$('a[name=delete-all]').click(function(evt) {
		...
		if(confirm('Estas seguro que quieres eliminar '+selected.length+' productos?')) {
			...
			$.post('items.php', {
				...
				if(data == 'wrong') {
					die('170 Algo salió mal. Por favor, inténtelo de nuevo');
					return false;
				}
	...
	function check_item(act, t) {
		...
		if($('input[type=checkbox][name=chbox][value="'+id+'"]').parent().parent().next().data('id') != 'y') {
			if(getViewportWidth() > 420)
				var cont = $("<tr style=\"display:none\" data-id=\"y\"><td></td><td></td><td></td><td>"+text+"</td><td><form method=\"post\" data-saveid=\""+id+"\" name=\""+act+"\"><input type=\"text\" class=\"num\" placeholder=\"Cant\" /></form></td><td><input type=\"submit\" value=\"Guardar\" class=\"btn blue save "+act+"\" /></td><td></td></tr>");
			else if(getViewportWidth() <= 420 && getViewportWidth() > 370)
				var cont = $("<tr style=\"display:none\" data-id=\"y\"><td></td><td></td><td>"+text+"</td><td></td><td><form method=\"post\" data-saveid=\""+id+"\" name=\""+act+"\"><input type=\"text\" class=\"num\" placeholder=\"Cant\" /></form></td><td></td><td><input type=\"submit\" value=\"Guardar\" class=\"btn blue save "+act+"\" /></td><td></td></tr>");
			else
				var cont = $("<tr style=\"display:none\" data-id=\"y\"><td></td><td></td><td>"+text+"</td><td></td><td><form method=\"post\" data-saveid=\""+id+"\" name=\""+act+"\"><input type=\"text\" class=\"num\" placeholder=\"Cant\" /></form></td><td></td><td><input type=\"submit\" value=\"Guardar\" class=\"btn blue save "+act+"\" /></td><td></td></tr>");
	...
	function submit_check(from, type, t) {
		...
		if(val == '0' || val == '') {
			alert('Por favor, inserte un número de elementos para comprobar '+type);
			return false;
		}
		
		if(type == 'out') {
			if(parseInt(prev_value) < val){
				alert('No puedes ver más de '+prev_value+' items');
				return false;
			}
		}
		...
		$.post('items.php', {
			...
			if(data == '2') {
				...
				alert('171 Algo salió mal. Por favor, inténtelo de nuevo');
			}
		});
	}
		...
		if(confirm('¿Está seguro de que desea eliminar este elemento?') == true) {
			...
			$.post('items.php', {
				...
				if(data == '1')
					...
					alert('172 Algo salió mal. Por favor, inténtelo de nuevo');
			});
		}
	});
	...
	$('form[name=searchf]').submit(function(evt) {
		...
		$.post('items.php', {
			...
			alert('173 Algo salió mal. Por favor, inténtelo de nuevo');
			$('.loader').fadeOut(200);
			return false;
		});
	});
